...
sudo -u build sh -c "repo-add -s local-repo/$REPO.db.tar.gz *.pkg.tar.xz"
aws s3 sync --follow-symlinks --acl public-read local-repo/ s3://$BUCKET/repo/x86_64/
aws s3 cp *.pkg.tar.xz s3://$BUCKET/repo/x86_64/
aws s3 cp *.pkg.tar.xz.sig s3://$BUCKET/repo/x86_64/
